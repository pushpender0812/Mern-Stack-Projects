...
const WishListItem = ({ wishLists }) => {
  ...
  const handleRemoveWishList = async (id) => {
    // console.log("id jkfj" ,id);
    try {
      const response = await fetch(
        `http://localhost:3000/api/user/remove-wishlist?user_id=${user.result._id}&product_id=${id._id}`,
        {
          method: "POST",
          headers: {
    ...
    <>
      {wishLists.map((item, index) => (
        <div className="col-sm-6 col-lg-4 col-xl-3" key={index} style={{width:"300px"}}>
          <button className="btn btn-danger" style={    {marginLeft:"250px"}} onClick={() => handleRemoveWishList(item.product_id)}>
         X
          </button>
          <div className="card shadow h-100">
            ...
            <div className="card-body pb-0">
              ...
              <p className="mb-2 text-truncate-2">
                Price: Rs.{item.product_id.product_price}
              </p>
