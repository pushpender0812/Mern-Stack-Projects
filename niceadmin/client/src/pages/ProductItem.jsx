...
const navigate = useNavigate()
  ...
  const addToCart = async (product) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success("WishList edit Successfully")
      } else {
  ...
  const setToggleWishlist = () => {
    const isInWishlist = wishList.find(
      (wishlistItem) =>
        wishlistItem.product_id._id.toString() === product._id.toString()
    );
    if (isInWishlist) {
      setToggle(true);
    } else {
  ...
  const handleRemoveWishList = async(id) => {
    console.log("id jkfj red heart clicked" ,id);
      ...
      if (response.ok) {
        ...
        setWishList((prevWishList) => prevWishList.filter(item => item.product_id._id !== id._id));
        setToggle(false)
