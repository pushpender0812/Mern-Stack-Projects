...
const loginUser = async (req, res) => {
  try {
    ...
    if (!useremail) {
      return res.status(400).send("invalid login email,Email Does not Exist");
    }
...
console.log("check cart",checkProduct);
    ...
    console.log(checkCart, "pushpu yadav");
...
const viewWishList = async (req, res) => {
  ...
  try {
    ...
    // console.log("list in backend",wishlist);
...
const viewCart = async (req, res) => {
  console.log("My name is pushpu");
  try {
    const user_id = req.userID;
    console.log("viewCart", user_id);
...
const viewMyOrders = async (req, res) => {
  try {
    ...
    const viewOrders = await Payment.find({ user_id: user_id })
      ...
      .populate("user_id")
      .populate("coupon_id");
    console.log(viewOrders, "sdfui viewing mt orders");
